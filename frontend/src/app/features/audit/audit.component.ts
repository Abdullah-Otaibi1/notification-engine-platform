...
@Component({
  selector: 'nep-audit',
  standalone: true,
  imports: [CommonModule, DatePipe, FormsModule, MatCardModule, MatTableModule, MatIconModule,
    MatProgressSpinnerModule, MatFormFieldModule, MatInputModule, MatButtonModule, MatPaginatorModule],
...
export class AuditComponent implements OnInit {
  ...
  columns = ['createdAt', 'username', 'role', 'action', 'resource', 'resourceId', 'reason'];
  ...
  load() {
    ...
    this.svc.getAll({ ...this.filter, page: this.page + 1, pageSize: this.pageSize }).subscribe({
      ...
      error: (err) => { this.error = err?.error?.error?.message ?? 'Failed to load audit logs'; this.loading = false; },
    });
  }
